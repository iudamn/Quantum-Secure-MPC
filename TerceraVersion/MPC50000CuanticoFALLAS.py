import time
import asyncio
import pandas as pd
from pathlib import Path
import glob
from collections import Counter
from mpyc.runtime import mpc
from qiskit import QuantumCircuit, transpile
from qiskit_aer import AerSimulator
import random

# Configuración de archivos CSV
BASE_DIR = Path(__file__).resolve().parent
CARPETA_DATOS = BASE_DIR / "CantidadDatos"
ARCHIVOS = glob.glob(str(CARPETA_DATOS / "2017_50000_*.csv"))

ESCALA = 1000  # Factor de escalado para los secretos

def print_shares_table(shares):
    print("\nTabla de shares por nodo:")
    print("Nodo | Share")
    print("-----|------")
    for i, s in enumerate(shares, start=1):
        print(f"  {i}  |  {s}")

def quantum_shamir_shares(secret: int, n: int, t: int) -> list:
    """Genera shares con Shamir usando superposición cuántica para los coeficientes"""
    coef = [secret]
    for _ in range(t - 1):
        qc = QuantumCircuit(1, 1)
        qc.h(0)
        qc.measure(0, 0)
        simulator = AerSimulator()
        tqc = transpile(qc, simulator)
        result = simulator.run(tqc, shots=1).result()
        measured = int(list(result.get_counts().keys())[0])
        coef_val = measured * 9 + 1
        coef.append(coef_val)
    shares = [sum(c * (x ** i) for i, c in enumerate(coef)) for x in range(1, n + 1)]
    return shares

def lagrange_interpolation(x_vals, y_vals):
    """Reconstruye el secreto mediante interpolación de Lagrange"""
    secret_recon = 0
    for i in range(len(y_vals)):
        term_val = y_vals[i]
        for j in range(len(y_vals)):
            if i != j:
                term_val *= x_vals[j] / (x_vals[j] - x_vals[i])
        secret_recon += term_val
    return round(secret_recon)

async def mpc_shamir_threshold_demo():
    start_time = time.time()

    print("Listado de archivos CSV encontrados:")
    for f in ARCHIVOS:
        print(f)

    await mpc.start()
    secint = mpc.SecInt()

    # Leer y contar todos los delitos
    all_delitos = []
    for archivo in ARCHIVOS:
        df = pd.read_csv(archivo, encoding='cp1252')
        delitos_col = df.iloc[:, 6].astype(str).tolist()
        all_delitos.extend(delitos_col)

    conteo = Counter(all_delitos)
    print("\nDelitos encontrados y conteo:")
    for d, c in conteo.items():
        print(f"{d}: {c}")

    # Configuración Shamir
    n_shares = 4
    thresholds = [2, 3, 4]  # Diferentes thresholds para probar tolerancia a fallas
    tabla_resultados = []

    for delito, cantidad in conteo.items():
        scaled_secret = cantidad * ESCALA
        # Generamos la cantidad máxima de shares necesaria
        shares = quantum_shamir_shares(scaled_secret, n_shares, max(thresholds))
        sec_shares = [secint(s) for s in shares]
        opened = [await mpc.output(s) for s in sec_shares]

        # Ensayar todos los thresholds y escenarios de fallas
        for t in thresholds:
            for lost in range(n_shares):
                available_shares = random.sample(opened, n_shares - lost)
                if len(available_shares) >= t:
                    secret_recon = lagrange_interpolation(range(1, t+1), available_shares[:t]) // ESCALA
                    correcto = secret_recon == cantidad
                else:
                    secret_recon = None
                    correcto = False
                tabla_resultados.append({
                    "Delito": delito,
                    "n_shares": n_shares,
                    "threshold": t,
                    "shares_perdidos": lost,
                    "secreto_reconstruido": secret_recon,
                    "correcto": correcto
                })

    await mpc.shutdown()
    end_time = time.time()
    print(f"\nTiempo total de ejecución: {end_time - start_time:.2f} segundos")

    # Guardar resultados en CSV
    df_resultados = pd.DataFrame(tabla_resultados)
    df_resultados.to_csv(BASE_DIR / "tabla_thresholds.csv", index=False)
    print("\nTabla de resultados guardada en 'tabla_thresholds.csv'")
    print(df_resultados)

if __name__ == "__main__":
    asyncio.run(mpc_shamir_threshold_demo())
